...
class TestUserMove(unittest.TestCase):
	...
	def setUp(self):
		self.game = oxo_data.newGame()

	def test_valid_user_move(self):
		cell = 0  # Choose the first cell
		result = oxo_data.userMove(self.game, cell)
	...
	def test_invalid_user_move(self):
		cell = 0
		oxo_data.userMove(self.game, cell)
...
class TestComputerMove(unittest.TestCase):
	...
	def test_computer_move(self):
		result = oxo_data.computerMove(self.game)
	...
	def test_winning_move_row(self):
		...
		for cell in cells:
			if cell == 0:
				...
				if cell % 2 == 1:
					oxo_data.computerMove(self.game)
				else:
					oxo_data.userMove(self.game, cell)
		result = oxo_data.userMove(self.game, 2)
...
class TestDrawCondition(unittest.TestCase):
	...
	def test_draw_condition(self):
		for i in range(9):
			if i % 2 == 0:
				oxo_data.userMove(self.game, i)
			else:
				oxo_data.computerMove(self.game)
		result = oxo_data._isWinningMove(self.game)
