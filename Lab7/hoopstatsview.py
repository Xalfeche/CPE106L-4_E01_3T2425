...
class HoopStatsView(EasyFrame):
    ...
    def __init__(self, frame):
        ...
        self.dataArea = self.addTextArea(text = self.frame.to_string(),
                                          row = 0, column = 0,
                                          columnspan = 2, height = 15)

        
        self.radioGroup = self.addRadiobuttonGroup(row = 1, column = 0)

        for col_name in self.frame.select_dtypes(include='number').columns:
            self.radioGroup.addRadiobutton(text=col_name, command=self.analyze)

        # Setting the first numeric column as the default selected button.
        first_numeric_col = self.frame.select_dtypes(include='number').columns[0]
        defaultRB = self.radioGroup._buttons[first_numeric_col]  # Access button by name
